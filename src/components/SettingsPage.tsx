              ...
              {(() => {
                ...
                if (isExporting === 'xingce') {
                  return MAIN_MODULES.map(m => {
                    ...
                    moduleQs.forEach(q => {
                      const tag = q.tags && q.tags.length > 0 ? q.tags[0] : '其他/未分类';
                    ...
                    return (
                      <div key={m} style={{ marginBottom: '40px' }}>
                        ...
                        {Object.entries(groupedByTag).map(([tagName, questions]) => (
                          <div key={tagName} style={{ marginBottom: '35px', paddingLeft: '10px' }}>
                            <div style={{ fontSize: '14px', fontWeight: 'black', color: '#4f46e5', marginBottom: '15px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  // Shenlun Grouping by Tags
                  const allTags = Array.from(new Set(filteredWrong.flatMap(q => q.reasonTags || ['未分类'])));
                  
                  return allTags.map(tag => {
                    const tagQs = filteredWrong.filter(q => (tag === '未分类' ? (!q.reasonTags || q.reasonTags.length === 0) : q.reasonTags?.includes(tag)));
      ...
      <section className="bg-white rounded-3xl p-5 border border-slate-100 shadow-sm space-y-4">
        ...
        <div className="space-y-3">
          {[
            ...
            { id: 'reasonTags', label: '基础错误原因 (平级)', color: 'rose' },
            { id: 'hierarchicalReasons', label: '分级错误因素 (树状)', color: 'indigo' }
          ].map(group => (
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] bg-white border border-slate-100 text-slate-400 px-1.5 py-0.5 rounded-md">
                    {group.id === 'reasonTags' 
                      ? Object.values(config.reasonTags).reduce((acc, val) => acc + val.length, 0)
                      : (config[group.id as keyof AppConfig] as string[]).length
                    } 个
                  </span>
                </div>
                ...
                {editingConfigType === group.id && (
                  ...
                  >
                    <div className="p-4 space-y-4">
                      {/* Module Selector for Reason Tags */}
                      {(group.id === 'reasonTags' || group.id === 'hierarchicalReasons') && (
                            ...
                            <button
                              ...
                              className={cn(
                                ...
                                selectedReasonModule === m 
                                  ? (group.id === 'hierarchicalReasons' ? "bg-indigo-500 text-white border-indigo-500" : "bg-rose-500 text-white border-rose-500") 
                      ...
                      {group.id === 'hierarchicalReasons' ? (
                        <div className="space-y-4">
                           <div className="flex items-start gap-2 bg-indigo-50 p-3 rounded-xl mb-2">
                             <Info size={14} className="text-indigo-400 shrink-0 mt-0.5" />
                             <p className="text-[10px] text-indigo-600 leading-relaxed font-black">
                        ...
                        <>
                          ...
                          <div className="flex gap-2">
                            <input 
                              ...
                              placeholder={group.id === 'reasonTags' ? `给${selectedReasonModule}添加标签...` : `添加新${group.label.slice(-2)}...`}
                          ...
                          <div className="flex flex-wrap gap-2">
                            {(group.id === 'reasonTags' 
                              ? (config.reasonTags[selectedReasonModule as string] || [])
