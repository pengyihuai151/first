...
export default function SettingsPage({ data, onUpdate, onNavigate }: { data: AppData; onUpdate: () => void; onNavigate: (tab: any) => void }) {
  ...
  const [baseUrl, setBaseUrl] = useState(localStorage.getItem('ai_base_url') || 'https://open.bigmodel.cn/api/paas/v4');
  const [model, setModel] = useState(localStorage.getItem('ai_model') || 'glm-4-flash');
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="bg-white p-4 rounded-2xl border border-slate-100 flex flex-col gap-3">
            <div className="flex justify-between items-start">
              <div className="bg-blue-50 w-fit p-2 rounded-xl text-blue-500">
                <Clock size={20} />
              </div>
              <button
                onClick={() => updateSettings({ screenWakeLockEnabled: !data.settings.screenWakeLockEnabled })}
                className={`w-12 h-6 rounded-full transition-colors relative ${
                  data.settings.screenWakeLockEnabled ? 'bg-amber-500' : 'bg-slate-200'
                }`}
              >
                <div 
                  className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full transition-transform ${
                    data.settings.screenWakeLockEnabled ? 'translate-x-6' : ''
                  }`}
                />
              </button>
            </div>
            <div className="text-left">
              <div className="text-sm font-bold">屏幕常亮</div>
              <div className="text-[10px] text-slate-400">学习时保持屏幕</div>
            </div>
          </div>
        </div>
      </div>
